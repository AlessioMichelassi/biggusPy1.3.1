...
class numPyImageGeneratorNode(AbstractNodeInterface):
    ...
    def showContextMenu(self, position):
        ...
        action7 = contextMenu.addAction("checkerBoardImage")
    ...
    def doCheckerBoardImage(self):
        # Crea un'immagine vuota con la dimensione desiderata
        checker_board_image = np.zeros((self.imageHeight, self.imageWidth, 3), dtype=np.uint8)

        checkSize = 50
        # Disegna le barre colorate
        for i in range(0, self.imageHeight, checkSize):
            for j in range(0, self.imageWidth, checkSize):
                if (i // checkSize + j // checkSize) % 2 == 0:
                    checker_board_image[i:i + checkSize, j:j + checkSize] = [255, 255, 255]
                else:
                    checker_board_image[i:i + checkSize, j:j + checkSize] = [0, 0, 0]
        self.changeInputValue(0, checker_board_image)
        return checker_board_image
